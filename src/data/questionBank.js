/**
 * Authentic Indian Road Traffic Signs & Regulations Question Bank
 * Sourced in accordance with the Motor Vehicles Act, 1988 & IRC (Indian Roads Congress) standards
 * Full English & Pure Hindi Support
 */
export const QUESTION_BANK = [
  {
    id: 1,
    question: "What is the mandatory action required when approaching a red octagonal STOP sign?",
    questionHi: "लाल अष्टकोणीय (8 भुजाओं वाले) 'STOP' (रुकिए) संकेत पर चालक को क्या करना अनिवार्य है?",
    options: [
      "Bring the vehicle to a complete halt before the stop line and proceed only when clear",
      "Slow down and continue if no vehicle is approaching from the right",
      "Sound horn and proceed at 20 km/h",
      "Stop only if emergency vehicles are in transit"
    ],
    optionsHi: [
      "स्टॉप लाइन से पहले वाहन को पूरी तरह रोकें और रास्ता साफ होने पर ही आगे बढ़ें",
      "गति धीमी करें और यदि दाईं ओर से कोई वाहन नहीं आ रहा हो तो चलते रहें",
      "हॉर्न बजाएं और 20 किमी/घंटा की गति से आगे बढ़ें",
      "केवल तभी रुकें जब आपातकालीन वाहन गुजर रहे हों"
    ],
    correctIndex: 0,
    explanation: "Under Section 119 of the Motor Vehicles Act, an octagonal STOP sign (IRC:67) mandates a 100% complete halt at the stop line before proceeding.",
    explanationHi: "मोटर वाहन अधिनियम की धारा 119 के तहत, अष्टकोणीय स्टॉप संकेत पर स्टॉप लाइन से पूर्व वाहन को 100% पूर्ण विराम देना कानूनी रूप से अनिवार्य है।",
    signEmoji: "🛑",
    shape: "Octagonal",
    shapeHi: "अष्टकोणीय",
    category: "Mandatory / Regulatory",
    categoryHi: "अनिवार्य / नियामक संकेत",
    lawSection: "Section 119, MV Act 1988"
  },
  {
    id: 2,
    question: "What does a circular sign with a red border containing the numeral '50' indicate?",
    questionHi: "लाल किनारे वाले गोलाकार संकेत के अंदर '50' लिखा होने का क्या अर्थ है?",
    options: [
      "Minimum permissible speed is 50 km/h",
      "Maximum legal speed limit is 50 km/h",
      "Next petrol pump is 50 km ahead",
      "Axle weight capacity limit is 50 quintals"
    ],
    optionsHi: [
      "न्यूनतम अनुमेय गति 50 किमी/घंटा है",
      "अधिकतम कानूनी गति सीमा 50 किमी/घंटा है",
      "अगला पेट्रोल पंप 50 किमी आगे है",
      "धुरी भार क्षमता सीमा 50 क्विंटल है"
    ],
    correctIndex: 1,
    explanation: "Circular signs with red borders signify prohibition or mandatory limits. The number indicates the maximum legal speed limit under standard conditions.",
    explanationHi: "लाल गोल घेरे वाले संकेत निषेध या सीमा दर्शाते हैं। इसके भीतर की संख्या सड़क पर अधिकतम कानूनी गति सीमा (50 किमी/घंटा) को दर्शाती है।",
    signEmoji: "⭕",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Speed Restriction",
    categoryHi: "अनिवार्य / गति सीमा",
    lawSection: "Section 112, MV Act 1988"
  },
  {
    id: 3,
    question: "What does an equilateral triangle pointing upwards with a pedestrian zebra crossing icon warn drivers about?",
    questionHi: "ऊपर की ओर नुकीले लाल त्रिकोण में जेब्रा क्रॉसिंग का चिन्ह क्या चेतावनी देता है?",
    options: [
      "Pedestrian zebra crossing ahead; yield right-of-way to pedestrians",
      "Pedestrian movement is strictly prohibited",
      "Foot-over bridge available ahead",
      "Designated jogging track zone"
    ],
    optionsHi: [
      "आगे पैदल पार पथ (जेब्रा क्रॉसिंग) है; पैदल यात्रियों को रास्ता दें",
      "पैदल चलना सख्त मना है",
      "आगे फुट-ओवर ब्रिज उपलब्ध है",
      "जॉगिंग ट्रैक क्षेत्र"
    ],
    correctIndex: 0,
    explanation: "Upward triangular signs with red borders are Cautionary Signs. Drivers must decelerate and give precedence to pedestrians crossing.",
    explanationHi: "ऊपर की ओर त्रिकोणीय संकेत सचेतक (चेतावनी) होते हैं। चालकों को गति धीमी करनी चाहिए और पैदल पार कर रहे यात्रियों को पहला अधिकार देना चाहिए।",
    signEmoji: "🚶",
    shape: "Triangular",
    shapeHi: "त्रिकोणीय",
    category: "Cautionary / Warning",
    categoryHi: "सचेतक / चेतावनी संकेत",
    lawSection: "Rule 11, Rules of the Road Regulations"
  },
  {
    id: 4,
    question: "What does a blue circular sign with a white arrow pointing straight ahead command?",
    questionHi: "नीले गोल घेरे में सीधा तीर का सफेद निशान क्या आदेश देता है?",
    options: [
      "Compulsory ahead only; no turns permitted",
      "One way traffic in the opposite direction",
      "Narrow bridge ahead",
      "Lane change recommended"
    ],
    optionsHi: [
      "केवल सीधा जाना अनिवार्य है; मुड़ना मना है",
      "विपरीत दिशा में एकतरफा यातायात",
      "आगे संकरा पुल है",
      "लेन बदलना अनुशंसित है"
    ],
    correctIndex: 0,
    explanation: "Blue circular signs without red slashes are Positive Mandatory Direction signs. Traffic must travel strictly in the indicated arrow direction.",
    explanationHi: "नीले गोल संकेत अनिवार्य सकारात्मक दिशा का आदेश देते हैं। वाहन को अनिवार्य रूप से केवल सीधे ही आगे बढ़ना होगा।",
    signEmoji: "⬆️",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Positive Direction",
    categoryHi: "अनिवार्य / दिशा निर्देश",
    lawSection: "IRC:67 Standard Sign"
  },
  {
    id: 5,
    question: "What does a circular sign showing an automobile horn crossed by a red diagonal slash signify?",
    questionHi: "हॉर्न के चित्र पर लाल तिरछी रेखा वाले गोलाकार संकेत का क्या अर्थ है?",
    options: [
      "Horn prohibited / Silence zone (near hospitals, schools, courts)",
      "Sound horn continuously while driving",
      "Horn testing area ahead",
      "Defective horns are penalized"
    ],
    optionsHi: [
      "हॉर्न बजाना मना है / शांत क्षेत्र (अस्पताल, न्यायालय आदि के पास)",
      "वाहन चलाते समय लगातार हॉर्न बजाएं",
      "आगे हॉर्न परीक्षण क्षेत्र है",
      "दोषपूर्ण हॉर्न पर जुर्माना है"
    ],
    correctIndex: 0,
    explanation: "A crossed-out horn indicates a designated Silence Zone. Honking within 100 meters of hospitals, educational institutions, and courts is an offence.",
    explanationHi: "कटा हुआ हॉर्न शांत क्षेत्र (साइलेंस जोन) दर्शाता है। अस्पताल, स्कूल या कोर्ट के 100 मीटर के भीतर हॉर्न बजाना धारा 194F के तहत दंडनीय है।",
    signEmoji: "🔇",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Prohibitory",
    categoryHi: "अनिवार्य / निषेध संकेत",
    lawSection: "Noise Pollution Rules & MV Act"
  },
  {
    id: 6,
    question: "What is the meaning of an inverted (downward-pointing) triangular sign with a red border?",
    questionHi: "उल्टे (नीचे की ओर नुकीले) लाल त्रिकोण वाले संकेत का क्या अर्थ है?",
    options: [
      "Give Way / Yield right of way to traffic on the major road",
      "Dead end / No thoroughfare ahead",
      "Rest area ahead",
      "Toll plaza approaching"
    ],
    optionsHi: [
      "रास्ता दीजिए (GIVE WAY) / मुख्य सड़क के वाहनों को प्राथमिकता दें",
      "आगे रास्ता बंद है",
      "आगे विश्राम स्थल है",
      "टोल प्लाजा आ रहा है"
    ],
    correctIndex: 0,
    explanation: "The inverted triangle is universally recognized as 'GIVE WAY'. It assigns priority to traffic already moving on the priority carriageway.",
    explanationHi: "उल्टा त्रिकोण 'रास्ता दें' (GIVE WAY) का संकेत है। यह मुख्य सड़क पर पहले से चल रहे वाहनों को प्राथमिकता देने का आदेश देता है।",
    signEmoji: "🔻",
    shape: "Inverted Triangle",
    shapeHi: "उल्टा त्रिकोण",
    category: "Mandatory / Priority Control",
    categoryHi: "अनिवार्य / प्राथमिकता नियंत्रण",
    lawSection: "IRC:67 Mandatory Series"
  },
  {
    id: 7,
    question: "What does a circular sign with a U-turn arrow crossed by a red diagonal slash indicate?",
    questionHi: "यू-टर्न के तीर पर लाल तिरछी रेखा वाले संकेत का क्या मतलब है?",
    options: [
      "U-Turn is strictly prohibited",
      "Compulsory U-turn ahead",
      "Roundabout ahead with right-of-way",
      "Sharp left hairpin bend ahead"
    ],
    optionsHi: [
      "यू-टर्न (वापस मुड़ना) पूर्णतः प्रतिबंधित है",
      "आगे अनिवार्य यू-टर्न है",
      "आगे गोल चक्कर है",
      "आगे तीव्र बायां मोड़ है"
    ],
    correctIndex: 0,
    explanation: "A red slash across a U-turn arrow prohibits turning the vehicle 180 degrees at that junction or road cut.",
    explanationHi: "यू-टर्न पर लाल तिरछी रेखा उस चौराहे या डिवाइडर कट पर वाहन को 180 डिग्री वापस मोड़ने से प्रतिबंधित करती है।",
    signEmoji: "↩️",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Prohibitory",
    categoryHi: "अनिवार्य / निषेध संकेत",
    lawSection: "Section 119, MV Act 1988"
  },
  {
    id: 8,
    question: "What does a cautionary triangular sign showing a hump on the road surface indicate?",
    questionHi: "त्रिकोण के अंदर सड़क पर उभार (हम्प) का चिन्ह क्या दर्शाता है?",
    options: [
      "Speed breaker / Rough road hump ahead; reduce speed immediately",
      "Mountain terrain beginning",
      "Railway level crossing without barrier",
      "Dual road merge ahead"
    ],
    optionsHi: [
      "आगे स्पीड ब्रेकर (गति अवरोधक) है; तुरंत गति धीमी करें",
      "पहाड़ी इलाका शुरू हो रहा है",
      "बिना फाटक का रेलवे क्रॉसिंग",
      "दो सड़कें आगे मिल रही हैं"
    ],
    correctIndex: 0,
    explanation: "This cautionary sign alerts motorists to reduce vehicle velocity to safely negotiate a speed breaker or uneven surface.",
    explanationHi: "यह चेतावनी संकेत वाहन चालकों को सचेत करता है कि आगे गति अवरोधक है ताकि वे समय रहते वाहन की गति नियंत्रित कर सकें।",
    signEmoji: "⚠️",
    shape: "Triangular",
    shapeHi: "त्रिकोणीय",
    category: "Cautionary / Road Condition",
    categoryHi: "सचेतक / सड़क की स्थिति",
    lawSection: "IRC Road Safety Guidelines"
  },
  {
    id: 9,
    question: "What does a circular red sign with a central horizontal white bar signify?",
    questionHi: "लाल गोल घेरे में बीच में सफेद क्षैतिज पट्टी का क्या अर्थ है?",
    options: [
      "No Entry for all vehicular traffic",
      "One way traffic to the right",
      "Speed check camera zone",
      "Commercial vehicles restricted"
    ],
    optionsHi: [
      "प्रवेश निषेध (NO ENTRY) / सभी वाहनों का आना वर्जित है",
      "दाईं ओर एकतरफा यातायात",
      "गति निगरानी कैमरा क्षेत्र",
      "वाणिज्यिक वाहन प्रतिबंधित"
    ],
    correctIndex: 0,
    explanation: "A solid red disc with a horizontal white bar is the standard 'NO ENTRY' sign. Entry into that road is strictly forbidden.",
    explanationHi: "सफेद पट्टी युक्त लाल वृत्त मानक 'नो एंट्री' संकेत है। उस सड़क पर वाहन ले जाना कानूनन वर्जित है।",
    signEmoji: "⛔",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Prohibitory",
    categoryHi: "अनिवार्य / प्रवेश निषेध",
    lawSection: "Section 115, MV Act 1988"
  },
  {
    id: 10,
    question: "When are you legally permitted to overtake a vehicle on its left side in India?",
    questionHi: "भारत में किसी वाहन को बाईं ओर से ओवरटेक करने की अनुमति कब होती है?",
    options: [
      "When the vehicle ahead is signaling and making a right turn in an open multi-lane road",
      "Whenever the right lane is congested",
      "At night when flashing headlights",
      "Left overtaking is never permissible under any circumstance"
    ],
    optionsHi: [
      "जब आगे वाला वाहन दाईं ओर मुड़ने का संकेत दे रहा हो और दाईं ओर मुड़ रहा हो",
      "जब भी दाहिनी लेन में भीड़ हो",
      "रात में हेडलाइट चमकाते समय",
      "किसी भी परिस्थिति में बाईं ओर से ओवरटेक नहीं किया जा सकता"
    ],
    correctIndex: 0,
    explanation: "According to the Rules of the Road Regulations (1989), overtaking on the left is permitted only when the front vehicle indicates its intention to turn right.",
    explanationHi: "सड़क नियमावली 1989 के नियम 14 के अनुसार, बाईं ओर से ओवरटेक केवल तभी किया जा सकता है जब आगे का वाहन दाईं ओर मुड़ने का स्पष्ट संकेत दे रहा हो।",
    signEmoji: "🚗",
    shape: "Rule-Based",
    shapeHi: "नियम आधारित",
    category: "Traffic Regulation Rule",
    categoryHi: "यातायात नियम विनियम",
    lawSection: "Rule 14, Rules of the Road Regulations"
  },
  {
    id: 11,
    question: "What does a triangular cautionary sign showing two children with school bags indicate?",
    questionHi: "त्रिकोण में स्कूल बैग लिए दो बच्चों का चिन्ह क्या चेतावनी देता है?",
    options: [
      "School zone ahead; drive with extreme caution and reduce speed",
      "Children playground area; parking permitted",
      "Pedestrian footpath closed",
      "Public park entrance"
    ],
    optionsHi: [
      "आगे स्कूल क्षेत्र है; गति धीमी करें और अत्यंत सतर्कता से वाहन चलाएं",
      "बच्चों का खेल मैदान; पार्किंग की अनुमति है",
      "पैदल फुटपाथ बंद है",
      "सार्वजनिक पार्क का प्रवेश द्वार"
    ],
    correctIndex: 0,
    explanation: "Warns drivers that children may suddenly cross the road. Drivers must slow down to school zone speed limits (usually 25 km/h).",
    explanationHi: "यह चालकों को सचेत करता है कि आगे विद्यालय है और बच्चे सड़क पार कर सकते हैं। गति 25 किमी/घंटा या उससे कम रखें।",
    signEmoji: "🚸",
    shape: "Triangular",
    shapeHi: "त्रिकोणीय",
    category: "Cautionary / Warning",
    categoryHi: "सचेतक / चेतावनी संकेत",
    lawSection: "School Safety Guidelines, MoRTH"
  },
  {
    id: 12,
    question: "What does a circular sign with a blue background and a white bicycle symbol specify?",
    questionHi: "नीले गोल घेरे में सफेद साइकिल का चिन्ह क्या आदेश देता है?",
    options: [
      "Compulsory cycle track / Dedicated bicycle lane only",
      "Cycles prohibited",
      "Motorcycle lane",
      "Bicycle repair shop ahead"
    ],
    optionsHi: [
      "अनिवार्य साइकिल मार्ग / केवल साइकिल चालकों के लिए आरक्षित लेन",
      "साइकिल चलाना मना है",
      "मोटरसाइकिल लेन",
      "आगे साइकिल मरम्मत की दुकान है"
    ],
    correctIndex: 0,
    explanation: "A blue circle indicates a compulsory path reserved exclusively for non-motorized pedal bicycles.",
    explanationHi: "नीला वृत्त अनिवार्य मार्ग दर्शाता है जो विशेष रूप से केवल साइकिल चालकों के लिए आरक्षित है। मोटर वाहन इसमें नहीं जा सकते।",
    signEmoji: "🚲",
    shape: "Circular",
    shapeHi: "गोलाकार",
    category: "Mandatory / Lane Reservation",
    categoryHi: "अनिवार्य / लेन आरक्षण",
    lawSection: "IRC:67 Mandatory Series"
  }
];
